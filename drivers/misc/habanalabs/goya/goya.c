			...
			struct hl_cs_parser *parser, bool is_mmu)
{
	...
	while (cb_parsed_length < parser->user_cb_size) {
		...
		user_pkt = (packet_t *) (uintptr_t)
			(parser->user_cb->kernel_address + cb_parsed_length);

		pkt_id = (enum packet_id) ((le64_to_cpu(user_pkt->packet_header) &
				...
				struct hl_cs_parser *parser)
{
	...
	while (cb_parsed_length < parser->user_cb_size) {
		...
		u16 pkt_size;
		u32 new_pkt_size = 0;
		packet_t *user_pkt, *kernel_pkt;

		user_pkt = (packet_t *) (uintptr_t)
			(parser->user_cb->kernel_address + cb_parsed_length);
		kernel_pkt = (packet_t *) (uintptr_t)
		...
		size_t irq_arr_size)
{
	...
	goya_irq_arr = (__le32*) &pkt->irqs;
	memcpy(goya_irq_arr, irq_arr, irq_arr_size);

	/* We must perform any necessary endianess conversation on the irq array
	 * being passed to the goya hardware. */
	for (irq_arr_index = 0; irq_arr_index < irq_num_entries; irq_arr_index++) {
		goya_irq_arr[irq_arr_index] = cpu_to_le32(goya_irq_arr[irq_arr_index]);
	}
